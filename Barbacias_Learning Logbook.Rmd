...
1.  Choose a location of interest, find out its latitude and produce plots of daily sunrise, sunset and daylength.
    ...
    ![](images/Total-number-of-weather-stations-per-country-or-territory-contained-in-the-GHCN-Daily_W640.jpg)
...
5.  Use the `patch_daily_temperatures` function to fill gaps
    ...
    #$`Stuttgart-Echterdingen`
     #    mean_bias stdev_bias filled gaps_remain
    #Tmin    -1.950      1.595   1027           0
    #Tmax    -2.352      1.383   1027           0
       

    ##thus no need to limit mean bias to 1 and stdevbias to 2
    write.csv(patched$weather, "data/MApfelstetten_patched.csv", row.names = FALSE)
...
6.  Investigate the results - have all gaps been filled?
    ...
    ```{r, eval=FALSE}
    library(chillR)
    patched<-read.csv("data/MApfelstetten_patched.csv")

    post_patch_stats<-fix_weather(patched)$QC

    write.csv(post_patch_stats, "data/MApfelstetten_post_patchstats.csv", row.names = FALSE)
    ```

    ```{r, echo=FALSE}
    library(kableExtra)

    post_patch_stats<-read_tab("data/MApfelstetten_post_patchstats.csv")
...
7.  If necessary, repeat until you have a dataset you can work with in further analyses
    ...
    Since everything has been filled, no need to repeat procedure. What I wish to repeat though is that I will not use the produced dataset for further exercises.
...
1.  For the location you chose for previous exercises, produce historic temperature scenarios representing several years of the historic record (your choice).
    ...
    ![](images/Annual_temp_Germany.jpg)
...
1.  Repeat the `PLS_chill_force` procedure for the 'Roter Boskoop' dataset. Include plots of daily chill and heat accumulation.
    ...
    Above is the PLS analysis plot (using CP model) applied to the Roter-Boskoop dataset. From here, I can roughly say that chilling period started around early December to February, which is quite similar with the example given in class. Forcing period must be around January to bloom date.
...
4.  Plot results for all three analyses, including shaded plot areas for the chilling and forcing periods you estimated.
...
5.  Evaluate how the risk of spring frost for this cultivar has changed over time. Has there been a significant trend?
    ...
    Trend above is hard to interpret, I will not attempt to. And after checking via Kendall test, trend is insignificant (p-value \>0.05) anyway.
...
-   *loading bars with %progress and time estimates*- I am not sure if this is entirely possible, but I am suggesting 'time estimates' anyway\--would be very helpful in deciding whether one should take a nap while waiting or not.

-    *update lecture portion in module (html) concurrently with the little edits done in the exercises portion*-since I am one of those not-so-conventional "studious" students who do exercises in pace with class (which I deeply regretted in the end), I had to rely on notes and modules. In some exercises, some datasets were mentioned, and there were phrases like "that you used in previous chapter", but they were not even mentioned in the previous chapter module. Bit confusing but figured out stuff by revisiting notes and Eike's github.

-   in Chapter 14 there is a *persistent error with the 'getClimateWizardData' function* when dealing with 2080 and 2085 specifically, for RCP8.5 for my location (48.4931° N, 9.3998° E) but it works with the other years (2050 and 2090, for example). It shows the error below.

    I tried comparing the outputs and all, but my guess is that there is a special range of temperature considered for temperature scenarios that might exclude some datasets/fail to recognize them. Had to improvise and use trial and error for the year. I learned a lot here though.
...
-   I got stuck with *Chapter 10* too, since `patch_daily_temps` seemed to only work perfectly fine with 'gsod' datafiles. So many hiccups because I could not also clean the 'gsod' raw files with `handle_gsod` for some reason. It always returns an empty file even when I tested the Bonn weather codes and samples from class. That was the reason I had to use 'dwd' datafiles and move the location to Swabian Orchard paradise. After so many attempts, I was able to figure out that the 'old and deprecated' function `patch_daily_temperatures` would work in my case.

With every word, I felt the passion that the module creators had while crafting the modules. It was very well-made to the point that someone clueless like I exited the course with huge progress in terms of R knowledge. I want more exposure to these concepts so I will be seeing the team again next semester for the Decisions Analysis and Forecasting subject. I will strive to do better for sure. *Bis bald!*
...
Luedeling, Eike, Evan H Girvetz, Mikhail A Semenov, and Patrick H Brown. 2011. \"Climate Change Affects Winter Chill for Temperate Fruit and Nut Trees.\" *PloS One* 6 (5): e20155. <https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0020155>.
...
*The Norns* (2018) *Norse Mythology for Smart People*. Available at: <https://norse-mythology.org/gods-and-creatures/others/the-norns/#:~:text=In%20Norse%20mythology%2C%20the%20Norns,any%20and%20all%20other%20beings.> (Accessed: March 15, 2023).

Tominaga, A. *et al.* (2022) \"How is global warming affecting Fruit Tree Blooming? \'Flowering (dormancy) disorder\' in Japanese pear (Pyrus pyrifolia) as a case study,\" *Frontiers in Plant Science*, 12. Available at: <https://doi.org/10.3389/fpls.2021.787638.>

*Schwäbisches Streuobstparadies* (no date) *Streuobstparadies*. Available at: <https://www.streuobstparadies.de/> (Accessed: March 15, 2023).
